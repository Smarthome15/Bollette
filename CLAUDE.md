...
- **Tab Andamento Prezzi — fattori tariffari** (`renderPrezziTab`, `computePrezziFattori`, `buildPrezziVerdetto`, `renderPrezziCharts`, `contaSegnalazioniPrezzi`, `getPrezziSoglia`, `getConsumoTipo`; `state.charts.prezzi/prezziFisso/prezziIndice`; rilavorata il 12/07/2026): tre serie SEPARATE per utenza dalle bollette con dati di dettaglio, ordinate per **competenza** — **quota fissa in €/giorno** (mai spalmata sul consumo), **prezzo unitario** (`prezzo_unitario_energia`, 3 decimali ovunque) e **indice "bolletta tipo"** in €/mese (`qf_giornaliera×30 + prezzo×consumo tipo`, confrontabile tra stagioni). Il **consumo tipo** è la mediana storica dalle autoletture, FISSA finché non la si cambia dall'input della tab (override per utenza in localStorage `consumicasa_consumo_tipo`, ↺ = torna alla mediana). Ogni fattore è confrontato con la bolletta **precedente** e col **pari periodo di un anno prima** (11–13 mesi, distingue stagionalità da rincaro vero); **verdetto colorato** in cima in linguaggio semplice e **soglia regolabile** (`prezzi-soglia`, default 15%). Guardie: quota fissa o date mancanti → fattore `null`, bolletta esclusa dai grafici che non può alimentare con nota del conteggio (mai zeri finti). La pagina **Verifica Anomalie** mostra un avviso cliccabile (`audit-prezzi-alert`) "N variazioni tariffarie da controllare" che rimanda qui. Dettagli in `docs/frontend.md` §5.
...
- ~~Rilavorare la tab Andamento Prezzi per fattori tariffari~~ — **fatta** il 12/07/2026 (vedi sezione frontend e `docs/frontend.md` §5; parere favorevole di Jarvis in bacheca con 3 note recepite: consumo tipo fisso, guardie sui dati mancanti, in prospettiva esporre l'indice come campo calcolato per il ponte REST). La serie gas era stata ripulita lo stesso giorno (prezzi mag-giu→1.133 e gen-feb→0.967 corretti via API sul Pi): tutti i record con `prezzo_unitario_energia` rispettano `consumo×prezzo≈quota_energia`.
