...
- **Rilavorare la tab Andamento Prezzi per fattori tariffari** (richiesta di Matteo, 12/07/2026): l'obiettivo vero è capire se i fattori "che ci sono sempre" aumentano in modo spropositato → segnale "è ora di cambiare operatore". Oggi la tab confronta solo il prezzo unitario della parte variabile; la **quota fissa è estratta ma non monitorata**, e un costo/consumo mescolato è fuorviante (con consumi bassi la quota fissa gonfia il €/m³ apparente: 3,20 €/Smc d'estate vs 1,36 d'inverno a parità di tariffa). Progetto: trend SEPARATI per fattore — prezzo unitario variabile (€/unità), **quota fissa normalizzata al giorno** (€/giorno, mai spalmata sul consumo) — più un indice sintetico "**bolletta tipo a consumo costante**" (quota_fissa_giornaliera×giorni + prezzo_unitario×consumo standard, es. mediana storica mensile) con soglie di segnalazione per fattore. NB: la serie gas è stata ripulita il 12/07/2026 (prezzi mag-giu→1.133 e gen-feb→0.967 corretti via API sul Pi): tutti i record con `prezzo_unitario_energia` rispettano ora `consumo×prezzo≈quota_energia`.
- **Scrittura atomica dei JSON** (tmp + rename in `api_save_data`) — concordata con Jarvis in bacheca.
